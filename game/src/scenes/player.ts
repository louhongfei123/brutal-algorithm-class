...
export function refreshHandCards(
  ...
  for (let i = 0; i < hand.length; i++) {
    const cardContainer = ui.renderCard(
      scene,
      hand[i],
      200 * 2 + scene.cardWidth * i,
      550 * 2,
      scene.cardWidth,
      scene.cardHeight,
    );
...
export function setHandCardsInteractive(scene: CombatScene) {
  if (
    scene.currentCombat().getUnitOfThisTurn() === scene.currentCombat().player
  ) {
    ...
    scene.enermyCollider = scene.physics.add.overlap(
      scene.handCardGroup,
      scene.enermyContainer,
      overlapListener,
    );
    scene.playerCollider = scene.physics.add.overlap(
      scene.handCardGroup,
