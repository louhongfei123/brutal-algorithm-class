...
export default class RewardScene extends Phaser.Scene {
    ...
    create() {
        ...
        let receivingArea = this.add.image(ui.centerX(this), ui.centerY(this) * 1.666, "hand");
        receivingArea.setScale(0.666)
        receivingArea.setInteractive()
        this.physics.add.existing(receivingArea);

        let text = this.add.text(ui.centerX(this), ui.centerY(this) * 0.33,
            "请选择一张卡牌作为奖励")
        text.setOrigin(0.5);
        text.setFontSize(70)
        console.log(text);
        ...
        let card1 = ui.renderCard(
            this, this.rewards[0],
            ui.centerX(this) - 300, ui.centerY(this),
            200, 400
        )

        let card2 = ui.renderCard(
            this, this.rewards[1],
            ui.centerX(this), ui.centerY(this),
            200, 400
        )
        ...
        const card1Collider = this.physics.add.overlap(
            card1, receivingArea, overlapListener);
        const card2Collider = this.physics.add.overlap(
            card2, receivingArea, overlapListener);
        const card3Collider = this.physics.add.overlap(
            card3, receivingArea, overlapListener);
    }
