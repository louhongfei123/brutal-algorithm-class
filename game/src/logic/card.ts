...
export class Attack implements Card {
  ...
  ): { from: CardEffect; to: CardEffect } | errors.InvalidBehavior {
    return {
...
export class FollowUpAttack implements Card {
  ...
  effect(
    ...
    if (discardPile.length === 0) {
      return new errors.InvalidBehavior(
    ...
    if (last.kind !== CardCategory.AttackCard) {
      return new errors.InvalidBehavior(
        "FollowUpAttack: The previous used card is not an Attack Card",
      );
    }
...
abstract class SelfCard {
  ...
  abstract effect(
    input: EffectArguments,
  ): { from: CardEffect } | errors.InvalidBehavior;
}
...
function sameOrigin(
  input: EffectArguments,
): errors.InvalidBehavior | undefined {
